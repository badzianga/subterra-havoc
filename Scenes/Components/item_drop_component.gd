...
# To use, simply add it to enemy, set HealthComponent in export variable and
# fill drops array. After death, enemy will drop items depending on chances.
...
# Iterates over drops array and checks, if can spawn item. If so, initializes
# item's id and texture, then adds it to the world near the enemy's death
# coordinates.
